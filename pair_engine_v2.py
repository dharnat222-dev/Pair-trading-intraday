"""
pair_engine_v2.py - Advanced Pair Selection Engine
Filters: Sector, Correlation, Cointegration, Beta, Half-life, Hurst, ADF
"""
...
class PairEngineV2:
    def __init__(self, data: pd.DataFrame, sector_map: dict = None):
        ...
        Args:
            ...
            sector_map: Dict mapping symbol -> sector (e.g., {"RELIANCE": "Energy"})
        """
        self.data = data
    ...
    def _get_sector(self, symbol: str) -> str:
        """Get sector for a symbol, default to 'Unknown'"""
    ...
    def _calculate_half_life(self, spread: pd.Series) -> float:
        ...
        try:
            ...
            return 999  # Very long half-life
        except:
            return 999
    ...
    def analyze_pair(self, s1: str, s2: str) -> dict:
        ...
        try:
            ...
            # 4. Cointegration (ADF test on spread)
            adf_result = adfuller(spread, autolag='AIC')
            result['adf_pval'] = round(adf_result[1], 4)
            
            # 5. Cointegration (Johansen style)
            ...
            # 8. Current Z-Score (last 20 periods)
            ...
            score += min((1 - min(coint_pval, 0.1) / 0.1) * 25, 25)  # Max 25
            score += min((1 - min(adf_result[1], 0.1) / 0.1) * 20, 20)  # Max 20
            score += min((1 - min(h, 0.5) / 0.5) * 15, 15)  # Max 15
            score += min((1 - min(half_life, 50) / 50) * 10, 10)  # Max 10
            result['score'] = round(min(score, 100), 1)
            
            # 10. Signal
            result['signal'] = self._generate_signal(
                zscore.iloc[-1] if not zscore.empty else 0,
                h, coint_pval, adf_result[1]
            )
    ...
    def _generate_signal(self, zscore: float, hurst: float, coint_pval: float, adf_pval: float) -> str:
        """Generate signal based on all metrics"""
        # Check if pair is suitable for trading
        if hurst >= 0.5 or coint_pval >= 0.05 or adf_pval >= 0.05:
            return "⚪ POOR_QUALITY"
    ...
    def scan_pairs(self, filters: dict = None) -> list:
        ...
        Filters:
            same_sector: bool (default: True)
            min_correlation: float (default: 0.7)
            max_coint_pval: float (default: 0.05)
            max_adf_pval: float (default: 0.05)
            max_hurst: float (default: 0.5)
            max_half_life: float (default: 50)
            min_beta: float (default: 0.5)
